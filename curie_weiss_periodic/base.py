    ...
    def __init__(self, hopmat = np.eye(11), lattice_size=11, omega=0.0, \
                                      times = np.linspace(0.0, 10.0,100),\
                                       hx=0.0, hy=0.0, hz=0.0,\
                                        jx=0.0, jy=0.0, jz=1.0, amp = -25.0, hdc = 0.1, norm = 0.1, verbose=False):
        ...
        self.jmat = hopmat
        self.norm = norm #Change to kac norm later
...
def run_dyn(params,initstate):
    ...
    return {"t":params.times,"sx":sxdata,"sz":szdata}

if __name__ == '__main__':   
    #Power law decay of interactions
    paramdat = ParamData()
    run_dyn(paramdat,initstate)
